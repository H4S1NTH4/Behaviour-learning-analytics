...
import type { CaptureConfig, KeystrokeEvent, SessionInfo } from './types';
...
export class KeystrokeCapture {
  ...
  private startCapture(): void {
    ...
    const keyDownDisposable = this.editor.onKeyDown((e: any) => this.handleKeyEvent(e, 'keyDown'));
    const keyUpDisposable = this.editor.onKeyUp((e: any) => this.handleKeyEvent(e, 'keyUp'));

    this.disposables.push(keyDownDisposable, keyUpDisposable);
  ...
  public stopCapture(): void {
    ...
    // Dispose Monaco event listeners
  ...
  public grantConsent(): void {
    ...
    if (this.editor && !this.isCapturing) {
      this.startCapture();
    }
  }
